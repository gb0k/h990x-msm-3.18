...
#########################################################################
#
# Added for the H990* which need this fixup
#
  if ! $bin/fix-h990-cmdline /tmp/anykernel/boot-new.img;
  then ui_print " ";
    ui_print 'Unable to adjust bootimg command-line to fit, aborting!';
    exit 1;
  fi;
#########################################################################
